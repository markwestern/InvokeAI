...
def build_opt(post_data, seed, gfpgan_model_exists):
    ...
    setattr(opt, 'gfpgan_strength', float(
        post_data['gfpgan_strength']) if gfpgan_model_exists else 0)
    setattr(opt, 'upscale', [int(post_data['upscale_level']), float(
        post_data['upscale_strength'])] if post_data['upscale_level'] != '' else None)
    setattr(opt, 'progress_images', 'progress_images' in post_data)
    setattr(opt, 'seed', None if int(
        post_data['seed']) == -1 else int(post_data['seed']))
    setattr(opt, 'variation_amount', float(
        post_data['variation_amount']) if int(post_data['seed']) != -1 else 0)
...
class DreamServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/":
            ...
            if os.path.exists(log_file):
                with open(log_file, "r") as log:
                    for line in log:
                        ...
                        urlUpscaled = url.replace(
                            "img-samples", "upscaled").replace(".png", "u.png")
            ...
            is_in_cwd = os.path.commonprefix(
                (os.path.realpath(path), cwd)) == cwd
    ...
    def do_POST(self):
        ...
        def image_done(image, seed, upscaled=False, first_seed=-1, use_prefix=None):
            ...
            normalized_prompt = PromptFormatter(
                self.model, iter_opt).normalize_prompt()
            ...
            if not upscaled:
                ...
                self.wfile.write(bytes(json.dumps(
                    {'event': 'result', 'url': path,
                        'seed': seed, 'config': config}
                ) + '\n', "utf-8"))

            # control state of the "postprocessing..." message
            upscaling_requested = opt.upscale or opt.gfpgan_strength > 0
            # NB: Is this bad python style? It is typical usage in a perl closure.
            nonlocal images_generated
            # NB: Is this bad python style? It is typical usage in a perl closure.
            nonlocal images_upscaled
            if upscaled:
                images_upscaled += 1
            else:
                images_generated += 1
        ...
        try:
            if opt.init_img is None:
                ...
                try:
                    # Run img2img
                    self.model.prompt2image(
                        **vars(opt1), step_callback=image_progress, image_callback=image_done)
                finally:
